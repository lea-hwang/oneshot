<template>
  <b-modal id="modal-shot" size="xl" hide-header hide-footer centered>
            ...
            <div class="d-flex align-content-between flex-wrap w-100">
              ...
              <div class="w-100">
                <div class="d-flex justify-content-between">
                  ...
                  <div>
                    ...
                    <small class="text-muted" @click="moveDetail"
                      >#{{ shot.movie?.title }}</small
                    >
                  </div>
                  <div>
                    <!-- 수정 삭제 버튼 -->
                    <span v-if="currentUser.pk === shotUserPk">
                      ...
                      >
                        ...
                        <b-dropdown-item @click="clickUpdateShot"
                          >수정</b-dropdown-item
                        >
                        <b-dropdown-item @click="clickDeleteShot(shot.id)"
                          >삭제</b-dropdown-item
                        >
                      </b-dropdown>
                    </span>
              ...
              <div class="w-100">
                ...
                <small class="text-muted"
                  >{{ shot.like_cnt }}명이 좋아합니다</small
                >
                <hr class="m-0" />
                <div class="d-flex">
                  <!-- 좋아요 버튼 -->
                  <div
                    @click="clickLikeShot(shot.id, index, shotType)"
                    class="col-1 d-flex align-items-center mouse-cursor-custom"
                  >
                    <font-awesome-icon
                      v-show="isLike"
  ...
  methods: {
    ...
    moveProfile() {
      ...
      if (this.$route.path !== `/profile/${this.shotUserUsername}`) {
        this.$router.push({
          name: "profile",
          params: { username: this.shotUserUsername },
        });
      }
      this.$bvModal.hide("modal-shot");
    },
    moveDetail() {
      this.$router.push({
        name: "detail",
        params: { movieId: this.shot.movie.pk },
      });
      this.$bvModal.hide("modal-shot");
    },
  },
