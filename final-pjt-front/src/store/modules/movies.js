...
export default {
  ...
  getters: {
    ...
    movieScrollLock: (state) => state.movieScrollLock,
  },
  ...
  mutations: {
    ...
    SET_MOVIE_DETAIL_HEADER: (state, url_path) =>
      (state.movieDetailHeader = url_path),
    SET_MOVIE_LIKE: (state, movieIsLiked) =>
      (state.movieIsLiked = movieIsLiked),
    SET_MOVIE_LIEK_CNT: (state, like_cnt) => (state.movie.like_cnt = like_cnt),
    SET_MOVIE_TRAILER: (state, movieTrailer) =>
      (state.movieTrailer = movieTrailer),
    SET_MOVIE_COMMENTS: (state, comments) => (state.movieComments = comments),
    SET_POPULARMOVIES: (state, popularMovies) =>
    ...
    SET_SHOTESTMOVIES: (state, shotestMovies) =>
      (state.shotestMovies = shotestMovies),
    SET_MOVIEMAXPAGE: (state, movieMaxPage) =>
      (state.movieMaxPage = movieMaxPage),
    RESET_MOVIEPAGENUM: (state) => (state.moviePageNum = 0),
    ADD_MOVIE_PAGE_NUM: (state) => (state.moviePageNum += 1),
    ...
    SET_MOVIE_SCROLL_LOCK: (state) =>
      (state.movieScrollLock = !state.movieScrollLock),
  },
  ...
  actions: {
    ...
    addMovies({ commit, getters }) {
      // scrollLock이 안 걸려있을 때만 요청할 수 있음.
      if (!getters.movieScrollLock) {
        commit("SET_MOVIE_SCROLL_LOCK"); // lock 걸기
        axios({
          ...
          .then((res) => {
            ...
            commit("ADD_MOVIE_PAGE_NUM");
            commit("SET_MOVIE_SCROLL_LOCK"); // lock 풀기
          })
          .catch((err) => console.error(err.response));
      }
    },
